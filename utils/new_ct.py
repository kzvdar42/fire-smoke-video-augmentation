...
from utils.ct_utils import Rotations
...
class ColorTransfer:
    ...
    def lab_transfer(self, img_arr_in=None, img_arr_ref=None, in_mask=None, ratio=1):
        ...
        lab_out = lab_in
        lab_out[:, :, 0] = self.mean_std_transfer(img_arr_in=lab_in, img_arr_ref=lab_ref, in_mask=in_mask, ratio=ratio)[:, :, 0]
        img_arr_out = cv2.cvtColor(lab_out, cv2.COLOR_LAB2BGR)
        return img_arr_out

    def mean_std_transfer(self, img_arr_in=None, img_arr_ref=None, in_mask=None, ratio=1):
        ...
        if in_mask is None:
            ...
            mean_in = np.mean(img_arr_in[in_mask], axis=(0, 1), keepdims=True)
            std_in = np.std(img_arr_in[in_mask], axis=(0, 1), keepdims=True)
        mean_ref = np.mean(img_arr_ref, axis=(0, 1), keepdims=True)
        std_ref = np.std(img_arr_ref, axis=(0, 1), keepdims=True)
        img_arr_out = (img_arr_in - mean_in * ratio) * (std_ref / (std_in + self.eps)) + mean_ref * ratio
...
def color_transfer(source, target, ratio=1):
    """
    ...
    """
    in_mask  = target[:, :, 3] != 0 if target.shape[2] == 4 else None
    # img_arr_col = ColorTransfer().mean_std_transfer(img_arr_in=target[:, :, :3], img_arr_ref=source, in_mask=in_mask)
    img_arr_col = ColorTransfer().lab_transfer(img_arr_in=target, img_arr_ref=source, in_mask=in_mask, ratio=ratio)
    # img_arr_col = ColorTransfer().pdf_tranfer(img_arr_in=target[:, :, :3], img_arr_ref=source)
    result = Regrain().regrain(img_arr_in=target[:, :, :3], img_arr_col=img_arr_col)
    return result
